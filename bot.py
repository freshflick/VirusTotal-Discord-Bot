...
async def on_message(message):
    ...
    async def on_message(message):
        ...
        # Only handle DMs with file attachments
        if isinstance(message.channel, discord.DMChannel) and message.attachments:
            attachment = message.attachments[0]
            if attachment.filename.endswith(".zip"):
                await message.channel.send(f"File: {attachment.filename} received for checking")
            else:
                await message.channel.send("Please upload the file in the specified format.")

        # Let commands run in servers
...
async def upload(interaction: discord.Interaction, file: discord.Attachment):
    if interaction.guild is not None:
        ...
        print(f"Slash commands used in DM by {interaction.user.name}")
